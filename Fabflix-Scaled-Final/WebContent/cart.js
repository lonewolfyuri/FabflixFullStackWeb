...
function remove(movieId) {
    jQuery.ajax({
        dataType: "json",
        method: "GET",
        url: "api/remove?id=" + movieId,
        success: (resultData) => updatePage()
    });
}

function removeOne(movieId) {
    jQuery.ajax({
        dataType: "json",
        method: "GET",
        url: "api/removeOne?id=" + movieId,
        success: (resultData) => updatePage()
    });
}

function addOne(movieId) {
    jQuery.ajax({
        dataType: "json",
        method: "GET",
...
function updateCart() {
    let target = "api/update?";
    let quantityFields = document.getElementsByClassName("quan");
    console.log(quantityFields);
    let quanArr = Array.from(quantityFields);
    console.log(quanArr);
    let first = true;
    for (let ndx = 0; ndx < quanArr.length; ndx++) {
        let quan = quanArr[ndx];
        console.log(quan.value);
        if (!first) target += "&";
        first = false;
        target += quan.id + "=" + quan.value;
    }
...
function handleCartResults(resultData) {
    ...
    if(resultData["empty"]) totalBodyElement.append("<tr><th>0</th><th> $ 0.00 </th><th> $ 0.00 </th><th> $ 0.00 </th></tr>");
        ...
        for (let ndx = 0; ndx < resultData["movies"].length; ndx++) {
            ...
            rowHTML += "<th><button value='" + curProd["id"] + "' onclick='removeOne(this.value)'> - </button></th>";
            rowHTML += '<th>' + curProd["quantity"] + '</th>';
            rowHTML += "<th><button value='" + curProd["id"] + "' onclick='addOne(this.value)'> + </button></th>";
            rowHTML += "<th><button value='" + curProd["id"] + "' class='remove' onclick='remove(this.value)'>Remove From Cart</button></th>";
